...
public class PlatformRegistry {
	...
	// Get platform by its configured name
	public PlatformContext getPlatformByName(String name) {
		return platformMap.values().stream().filter(ctx -> ctx.getName().equalsIgnoreCase(name)).findFirst()
				.orElse(null);
	}

	// Get platform by enum type
	public PlatformContext getPlatformByType(PlatformType type) {
	...
	private void registerDefaultPlatforms() {
		...
		youtube.setElementsToRemove(
				Map.of("id", List.of("masthead-container", "header"), "tag", List.of("tp-yt-app-drawer")));
		...
		reddit.setElementsToRemove(Map.of("id", List.of("SHORTCUT_FOCUSABLE_DIV"), "tag", List.of("header")));
